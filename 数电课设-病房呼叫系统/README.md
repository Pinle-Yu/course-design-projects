...
- **PCB 设计（主要负责）：** 在嘉立创 EDA 中完成原理图整理、器件封装核对、电源连接和双层 PCB 布局布线，重点检查走线连通性、器件位置、接口方向和板面标识。
- **译码显示验证（主要负责）：** 使用 Multisim 对译码显示模块进行仿真，核对逻辑芯片之间的信号连接和数码管显示结果；其他功能模块的设计与实现由小组共同完成。
- **制板与联调（参与配合）：** 参与板卡焊接、上电检查和整机测试，并根据测试现象协助排查连接和显示问题。
