...
- **译码显示仿真（主要负责）：** 使用 Multisim 验证译码显示模块，核对逻辑芯片之间的信号连接和数码管显示结果；其他功能模块由小组共同完成。
- **PCB 设计与焊接（主要负责）：** 在嘉立创 EDA 中完成原理图整理、器件封装核对、电源连接和双层 PCB 布局布线，重点检查走线连通性、器件位置、接口方向和板面标识；完成板卡焊接和上电检查。
