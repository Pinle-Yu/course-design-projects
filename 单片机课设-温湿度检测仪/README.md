...
单片机与嵌入式系统课程设计。项目由小组共同完成，本人担任组长，主要负责 PCB 设计和休眠代码编写，并参与整机联调。

## 个人工作

- **PCB 设计（主要负责）：** 完成原理图与 PCB 设计，核对 STM32G030K6T6、SHT40、SN74HC595、数码管和按键接口的引脚连接，检查板层走线、器件布局及可焊接性。
- **休眠代码（主要负责）：** 编写并整合空闲状态下暂停 SysTick、进入 Sleep，以及 PB5 外部中断唤醒后恢复系统节拍的代码流程。
- **整机联调（参与配合）：** 配合组员完成温湿度采集、数码管显示、贴片焊接和程序烧录后的功能测试，协助确认唤醒、显示和重新休眠流程。
